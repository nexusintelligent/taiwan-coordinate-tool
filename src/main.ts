...
import {
  ...
  duplicateSelected, exportGeoJson, exportKml, featurePresets, featureSummary, finishDrawing, fitFeatures, fitReferenceLayer, fitWorkFeatures, getWorkFeatures, importGeoJson, isReferenceLayerVisible, layerFeatures, locateMap,
  removeLastDrawPoint, replaceWorkGeoJson, referenceLayerDefinitions, selectAllFeatures, selectFeature, selectedFeature, selectedFeatures,
  ...
  setReferenceLayerVisible, toggleFeatureSelection, updateSelected, visibleReferenceLegendItems, type EditorMode, type FeaturePreset, type LayerDefinition, type TbnRecord,
} from "./map";
...
app.innerHTML = `
  ...
  <main class="app-shell">
    <aside class="workbench"><button id="workbenchResize" class="workbench-resize" title="拖曳調整左側寬度" aria-label="調整左側工作區寬度"></button>
      ...
      <section id="editorView" class="view-panel">
        ...
        <div class="editor-card">
          ...
          <details class="coordinate-entry"><summary>以精確座標新增目前工具的圖徵</summary><div><label>座標（線、面請每行一組）<textarea id="coordinateGeometryInput" rows="3" placeholder="24.1243378, 120.6764627&#10;24.1250000, 120.6770000"></textarea></label><button id="addCoordinateGeometry" class="small-button">依目前新增點／線／面工具建立</button><span id="coordinateGeometryMessage"></span></div></details>
        ...
        <div class="editor-card reference-card">
          <div class="card-heading"><h3>預設參考圖層</h3><span>點選時才載入</span></div>
          <p class="hint">各圖層可獨立開關；資料已轉為 WGS84 並針對網頁顯示簡化。</p>
          <div class="reference-layer-list">${[...new Set(referenceLayerDefinitions.map((item)=>item.group))].map((group)=>`<details data-reference-group="${group}"><summary><input type="checkbox" data-reference-group-toggle="${group}" aria-label="開關${group}"><span>${group}</span></summary>${referenceLayerDefinitions.filter((item)=>item.group===group).map((item)=>`<label data-reference-layer-row="${item.id}"><input type="checkbox" data-reference-layer="${item.id}"><i style="--reference-color:${item.color}"></i><span>${item.name}</span></label>`).join("")}</details>`).join("")}</div>
        </div>
        <div class="editor-card">
          <h3>資料交換</h3>
          <p class="hint">GeoJSON 使用 WGS84 儲存，匯入後自動投影到地圖；可再由 QGIS 另存為 EPSG:3826。</p>
          <div class="button-row wrap"><label class="file-button">匯入圖資<input id="spatialFile" type="file" accept=".geojson,.json,.kml,.zip,.kmz,.7z,.rar,.shp,.shx,.dbf,.prj,.cpg" multiple hidden></label><select id="exportFormat" aria-label="匯出格式"><option value="geojson">GeoJSON</option><option value="kml">KML</option><option value="shp">Shapefile ZIP</option></select><button id="exportSpatial" class="small-button">匯出圖資</button><button id="clearFeatures" class="danger-button">清空工作圖層</button><button id="clearSession" class="danger-button">清除已保存工作階段</button></div>
          <button id="openLayout" class="small-button">建立圖面配置</button>
          <p class="hint">支援 GeoJSON、KML、標準 ZIP Shapefile；散檔請一次選取或拖入同名 SHP、SHX、DBF、PRJ、CPG。7z、RAR、KMZ 會辨識格式並提示安全的轉換方式。</p>
        </div>
    ...
    <section class="map-workspace">
      <nav class="desktop-menu" aria-label="圖資功能選單">
        <div class="desktop-menu-group"><button class="desktop-menu-trigger" aria-expanded="false">檔案</button><div class="desktop-submenu"><button data-menu-action="import">匯入圖資…</button><button data-menu-action="export">匯出圖資…</button><button data-menu-action="layout">圖面配置…</button></div></div>
        <div class="desktop-menu-group"><button class="desktop-menu-trigger" aria-expanded="false">編輯</button><div class="desktop-submenu"><button data-menu-action="undo">復原</button><button data-menu-action="redo">重做</button><button data-menu-action="delete">刪除選取圖徵</button></div></div>
        <div class="desktop-menu-group"><button class="desktop-menu-trigger" aria-expanded="false">檢視</button><div class="desktop-submenu"><button data-menu-action="fit">縮放至全部圖徵</button><button data-menu-action="toggle-workbench">顯示／隱藏左側工作區</button><button data-menu-action="toggle-header">顯示／隱藏上方區域</button><button data-menu-action="theme">切換深淺色主題</button></div></div>
        <div class="desktop-menu-group"><button class="desktop-menu-trigger" aria-expanded="false">說明</button><div class="desktop-submenu"><button data-menu-action="shortcuts">快捷鍵說明</button><button data-menu-action="about">關於與免責聲明</button></div></div>
      </nav>
      <div class="map-topbar"><div><strong>圖資位置檢核</strong><span id="mapContext">座標成果預覽</span></div><div class="map-toolbar" aria-label="地圖工具列"><button data-map-tool="browse" title="拖曳圖面">✋</button><button data-map-tool="select" title="選取圖徵">⌖</button><button id="toolbarZoomAll" title="縮放至全部圖徵">⛶</button><button id="toolbarUndo" title="復原 Ctrl+Z">↶</button><button id="toolbarRedo" title="重做 Ctrl+Y">↷</button><button id="toolbarDelete" title="刪除選取圖徵">⌫</button></div><div class="map-controls"><select id="baseMap"><option value="emap">臺灣通用電子地圖</option><option value="photo">國土測繪中心正射影像</option></select><label><input id="showRegionalDrainage" type="checkbox">區域排水</label><label><input id="showResultLabels" type="checkbox" checked>點位標籤</label></div></div>
      ...
      <div class="map-status"><span id="cursorCoordinate">游標：—</span><div class="status-locator"><label for="locateInput">定位到</label><input id="locateInput" placeholder="經緯度或 TWD97 X, Y"><button id="locateButton">定位</button><span id="locateMessage"></span></div><span>專案基準：TWD97 / TM2 zone 121 · EPSG:3826</span></div>
    </section>
  </main>
  <div id="contextMenu" class="context-menu" role="menu" hidden></div>
  <dialog id="layoutDialog" class="layout-dialog"><form method="dialog"><div class="layout-settings"><h2>圖面配置</h2><label>圖名<input id="layoutTitle" value="工程圖資配置圖"></label><label>紙張<select id="layoutPaper"><option>A4 橫式</option><option>A4 直式</option></select></label><label><input id="layoutNorth" type="checkbox" checked>指北針</label><label><input id="layoutScale" type="checkbox" checked>比例尺</label><label><input id="layoutLegend" type="checkbox" checked>圖例</label><label>圖例內容（每行一項，可留空）<textarea id="layoutLegendText" rows="7" placeholder="工程計畫範圍&#10;調查範圍 100m"></textarea></label><button id="exportLayout" type="button">匯出 PNG</button><button>關閉</button></div><div id="layoutSheet" class="layout-sheet"><h1 id="layoutTitlePreview">工程圖資配置圖</h1><div id="layoutMapPreview" class="layout-map-preview"></div><div id="northArrow" class="north-arrow"><b>N</b><span></span></div><div id="scaleBar" class="scale-bar"><div class="scale-labels"><b>0</b><b>100</b><b>200 m</b></div><span></span></div><div id="layoutLegendPreview" class="layout-legend"><strong>圖例</strong></div><p>智聯工程科技顧問有限公司</p></div></form></dialog>
...
let contextLayerId = "";
let contextReferenceId = "";
let contextReferenceGroup = "";
...
let layoutLegendColors:string[]=[];
let layoutLegendColorByName=new Map<string,string>();
function currentMapLegendItems():Array<{name:string;color:string}>{ const items:Array<{name:string;color:string}>=[]; const seen=new Set<string>(); getWorkFeatures().forEach((feature)=>{ const layer=workLayers.find((item)=>item.id===feature.get("layerId")); if(layer&&!layer.visible)return; const preset=featurePresets[feature.get("preset") as FeaturePreset]; if(preset&&!seen.has(preset.label)){seen.add(preset.label);items.push({name:preset.label,color:preset.color});} }); visibleReferenceLegendItems().forEach((item)=>{if(!seen.has(item.name)){seen.add(item.name);items.push({name:item.name,color:item.color});}}); if((<HTMLInputElement>$("#showRegionalDrainage")).checked)items.push({name:"區域排水",color:"#1e6fb5"}); if(converted.length&&(<HTMLInputElement>$("#showResultLabels")).checked)items.push({name:"座標點位",color:"#efb750"}); return items; }
function seedLayoutLegend():void{ const items=currentMapLegendItems(); layoutLegendColors=items.map((item)=>item.color); layoutLegendColorByName=new Map(items.map((item)=>[item.name,item.color])); (<HTMLTextAreaElement>$("#layoutLegendText")).value=items.map((item)=>item.name).join("\n"); }
function layoutLegendColor(line:string,index:number):string{return layoutLegendColorByName.get(line)||layoutLegendColors[index]||["#f08a19","#153cff","#e32f35","#73a942"][index%4];}
function renderLayoutLegend(): void { const lines=(<HTMLTextAreaElement>$("#layoutLegendText")).value.split(/\r?\n/).map((line)=>line.trim()).filter(Boolean); $("#layoutLegendPreview").innerHTML=`<strong>圖例</strong>${lines.map((line,index)=>`<span><i style="--legend-color:${layoutLegendColor(line,index)}"></i>${escapeHtml(line)}</span>`).join("")}`; }
...
function toggleWorkbench(): void { document.body.classList.toggle("workbench-collapsed"); requestAnimationFrame(() => map.updateSize()); }
const savedWorkbenchWidth=Number(localStorage.getItem("zhilian-workbench-width")); if(savedWorkbenchWidth>=320&&savedWorkbenchWidth<=760) document.documentElement.style.setProperty("--workbench-width",`${savedWorkbenchWidth}px`);
$("#workbenchResize").addEventListener("pointerdown",(event)=>{ const start=(event as PointerEvent).clientX; const initial=(document.querySelector(".workbench") as HTMLElement).getBoundingClientRect().width; const move=(moveEvent:PointerEvent)=>{ const width=Math.max(320,Math.min(760,initial+moveEvent.clientX-start)); document.documentElement.style.setProperty("--workbench-width",`${width}px`); localStorage.setItem("zhilian-workbench-width",String(Math.round(width))); map.updateSize(); }; const finish=()=>{ window.removeEventListener("pointermove",move); window.removeEventListener("pointerup",finish); }; window.addEventListener("pointermove",move); window.addEventListener("pointerup",finish); });
function toggleHeader(): void { document.body.classList.toggle("header-collapsed"); requestAnimationFrame(() => map.updateSize()); }
const savedTheme = localStorage.getItem("zhilian-theme"); if (savedTheme) document.documentElement.dataset.theme = savedTheme;
function toggleTheme(): void { const theme = document.documentElement.dataset.theme === "dark" ? "light" : "dark"; document.documentElement.dataset.theme = theme; localStorage.setItem("zhilian-theme", theme); }
document.querySelectorAll<HTMLButtonElement>(".desktop-menu-trigger").forEach((button)=>button.addEventListener("click",(event)=>{ event.stopPropagation(); const group=button.parentElement!; document.querySelectorAll(".desktop-menu-group.open").forEach((item)=>{ if(item!==group)item.classList.remove("open"); }); group.classList.toggle("open"); button.setAttribute("aria-expanded",String(group.classList.contains("open"))); }));
document.querySelectorAll<HTMLButtonElement>("[data-menu-action]").forEach((button) => button.addEventListener("click", () => { document.querySelectorAll(".desktop-menu-group.open").forEach((item)=>item.classList.remove("open")); const action = button.dataset.menuAction; if (action === "import") (<HTMLInputElement>$("#spatialFile")).click(); else if(action==="export") $("#exportSpatial").dispatchEvent(new MouseEvent("click")); else if(action==="layout") $("#openLayout").dispatchEvent(new MouseEvent("click")); else if (action === "undo") undo(); else if(action==="redo") redo(); else if(action==="delete") removeFeatures(); else if (action === "fit") fitWorkFeatures(map); else if(action==="toggle-workbench") toggleWorkbench(); else if(action==="toggle-header") toggleHeader(); else if(action==="theme") toggleTheme(); else if(action==="shortcuts") document.querySelector<HTMLDetailsElement>(".shortcut-help")?.scrollIntoView({behavior:"smooth"}); else if(action==="about") document.querySelector("footer")?.scrollIntoView({behavior:"smooth"}); }));
...
function syncReferenceGroup(group:string): void { const children=[...document.querySelectorAll<HTMLInputElement>(`[data-reference-layer-row] input[data-reference-layer]`)].filter((input)=>referenceLayerDefinitions.find((item)=>item.id===input.dataset.referenceLayer)?.group===group); const toggle=document.querySelector<HTMLInputElement>(`[data-reference-group-toggle="${CSS.escape(group)}"]`); if(!toggle)return; toggle.checked=children.length>0&&children.every((input)=>input.checked); toggle.indeterminate=children.some((input)=>input.checked)&&!toggle.checked; }
function setReferenceGroupVisible(group:string,visible:boolean): void { referenceLayerDefinitions.filter((item)=>item.group===group).forEach((item)=>{ setReferenceLayerVisible(item.id,visible); const input=document.querySelector<HTMLInputElement>(`[data-reference-layer="${item.id}"]`); if(input)input.checked=visible; }); syncReferenceGroup(group); }
document.querySelectorAll<HTMLInputElement>("[data-reference-layer]").forEach((input)=>input.addEventListener("change",()=>{ setReferenceLayerVisible(input.dataset.referenceLayer!,input.checked); const group=referenceLayerDefinitions.find((item)=>item.id===input.dataset.referenceLayer)?.group; if(group)syncReferenceGroup(group); }));
document.querySelectorAll<HTMLInputElement>("[data-reference-group-toggle]").forEach((input)=>{ input.addEventListener("click",(event)=>event.stopPropagation()); input.addEventListener("change",()=>setReferenceGroupVisible(input.dataset.referenceGroupToggle!,input.checked)); });
...
$("#exportSpatial").addEventListener("click", async () => { if (!getWorkFeatures().length) { alert("工作圖層目前沒有可匯出的圖徵。"); return; } const date = new Date().toISOString().slice(0, 10); const type = (<HTMLSelectElement>$("#exportFormat")).value; if (type === "kml") download(`智聯工程圖資_${date}.kml`, exportKml(), "application/vnd.google-earth.kml+xml;charset=utf-8"); else if(type==="shp") { try { const {zip}=await import("@mapbox/shp-write"); const archive=await zip(JSON.parse(exportGeoJson()),{folder:"智聯工程圖資",outputType:"blob",compression:"STORE",types:{point:"points",polyline:"lines",polygon:"polygons"},prj:'GEOGCS["WGS 84",DATUM["WGS_1984",SPHEROID["WGS 84",6378137,298.257223563]],PRIMEM["Greenwich",0],UNIT["degree",0.0174532925199433]]'}); download(`智聯工程圖資_${date}.zip`,archive,"application/zip"); } catch(error){ alert(`Shapefile 匯出失敗：${(error as Error).message}`); } } else download(`智聯工程圖資_${date}.geojson`, exportGeoJson(), "application/geo+json;charset=utf-8"); });
$("#openLayout").addEventListener("click",()=>{ const canvas=captureMapCanvas(); $("#layoutMapPreview").setAttribute("style",`background-image:url(${canvas.toDataURL("image/png")})`); seedLayoutLegend(); renderLayoutLegend(); (<HTMLDialogElement>$("#layoutDialog")).showModal(); });
...
$("#layoutLegendText").addEventListener("input",()=>{ const lines=(<HTMLTextAreaElement>$("#layoutLegendText")).value.split(/\r?\n/).map((line)=>line.trim()).filter(Boolean); layoutLegendColors=lines.map((line,index)=>layoutLegendColor(line,index)); renderLayoutLegend(); });
$("#exportLayout").addEventListener("click",()=>{ const mapCanvas=captureMapCanvas(); const canvas=document.createElement("canvas"); canvas.width=1600; canvas.height=1100; const c=canvas.getContext("2d")!; c.fillStyle="#fff"; c.fillRect(0,0,1600,1100); c.fillStyle="#183b35"; c.font="bold 42px sans-serif"; c.textAlign="center"; c.fillText((<HTMLInputElement>$("#layoutTitle")).value,800,65); c.drawImage(mapCanvas,70,100,1460,850); if((<HTMLInputElement>$("#layoutNorth")).checked){c.fillStyle="#fff";c.strokeStyle="#111";c.lineWidth=6;c.beginPath();c.moveTo(1440,125);c.lineTo(1405,225);c.lineTo(1440,202);c.lineTo(1475,225);c.closePath();c.fill();c.stroke();c.fillStyle="#111";c.font="bold 24px sans-serif";c.fillText("N",1440,112);} if((<HTMLInputElement>$("#layoutScale")).checked){ c.fillStyle="#fff";c.fillRect(90,845,265,72);c.strokeStyle="#111";c.lineWidth=2;c.strokeRect(110,875,220,20);c.fillStyle="#111";c.fillRect(110,875,110,20);c.font="16px sans-serif";c.textAlign="left";c.fillText("0",108,870);c.fillText("100",205,870);c.fillText("200 m",298,870); } if((<HTMLInputElement>$("#layoutLegend")).checked){ const lines=(<HTMLTextAreaElement>$("#layoutLegendText")).value.split(/\r?\n/).map((line)=>line.trim()).filter(Boolean); if(lines.length){ const h=55+lines.length*34;c.fillStyle="#fffffff0";c.fillRect(1230,920-h,300,h);c.fillStyle="#111";c.font="bold 24px sans-serif";c.textAlign="left";c.fillText("圖例",1250,950-h);c.font="18px sans-serif";lines.forEach((line,index)=>{c.strokeStyle=layoutLegendColors[index]||["#f08a19","#153cff","#e32f35","#73a942"][index%4];c.lineWidth=5;c.beginPath();c.moveTo(1250,978-h+index*34);c.lineTo(1300,978-h+index*34);c.stroke();c.fillStyle="#111";c.fillText(line,1315,984-h+index*34);}); } } c.fillStyle="#111"; c.font="18px sans-serif"; c.textAlign="left"; c.fillText("智聯工程科技顧問有限公司",70,1060); canvas.toBlob((blob)=>blob&&download(`圖面配置_${new Date().toISOString().slice(0,10)}.png`,blob,"image/png")); });
...
["showWorkLabels", "showResultLabels"].forEach((id) => $(`#${id}`).addEventListener("change", () => setMapDisplay({ workLabels: ($("#showWorkLabels") as HTMLInputElement).checked, resultLabels: ($("#showResultLabels") as HTMLInputElement).checked })));
...
$(".reference-layer-list").addEventListener("contextmenu",(event)=>{ const target=event.target as HTMLElement; const row=target.closest<HTMLElement>("[data-reference-layer-row]"); const groupNode=target.closest<HTMLElement>("[data-reference-group]"); if(row){ contextReferenceId=row.dataset.referenceLayerRow!; const definition=referenceLayerDefinitions.find((item)=>item.id===contextReferenceId)!; const visible=isReferenceLayerVisible(contextReferenceId); openContextMenu(event as MouseEvent,[{action:"zoom-reference",icon:"⌖",label:"縮放至圖層"},{action:"toggle-reference",icon:visible?"◉":"○",label:visible?"隱藏圖層":"顯示圖層"},{action:"reference-info",icon:"ⓘ",label:`圖層資訊：${definition.name}`}]); return; } if(groupNode){ contextReferenceGroup=groupNode.dataset.referenceGroup!; const members=referenceLayerDefinitions.filter((item)=>item.group===contextReferenceGroup); const allVisible=members.every((item)=>isReferenceLayerVisible(item.id)); openContextMenu(event as MouseEvent,[{action:"show-reference-group",icon:"◉",label:"開啟群組全部圖層",disabled:allVisible},{action:"hide-reference-group",icon:"○",label:"關閉群組全部圖層",disabled:!members.some((item)=>isReferenceLayerVisible(item.id))}]); } });
$("#layerTree").addEventListener("contextmenu", (event) => {
  ...
  openContextMenu(event as MouseEvent, [{ action: "zoom-selection", icon: "⌖", label: "縮放至圖徵" }, { action: "rename-feature", icon: "✎", label: "重新命名" }, { action: "copy-feature", icon: "⧉", label: "複製" }, { action: "duplicate-feature", icon: "＋", label: "複製一份" }, { action: "delete-feature", icon: "⌫", label: "刪除圖徵", danger: true }]);
});
$("#map").addEventListener("contextmenu", (event) => {
  ...
  if(contextFeature?.get("kind")==="reference"){ contextReferenceId=String(contextFeature.get("referenceId")); const visible=isReferenceLayerVisible(contextReferenceId); openContextMenu(mouseEvent,[{action:"zoom-reference",icon:"⌖",label:"縮放至參考圖層"},{action:"toggle-reference",icon:visible?"◉":"○",label:visible?"隱藏參考圖層":"顯示參考圖層"},{action:"reference-info",icon:"ⓘ",label:`圖層資訊：${String(contextFeature.get("referenceName"))}`}]); return; }
  if (contextFeature?.get("layerId")) { selectFeature(contextFeature); populateProperties(contextFeature); openContextMenu(mouseEvent, [{ action: "zoom-selection", icon: "⌖", label: "縮放至圖徵" }, { action: "rename-feature", icon: "✎", label: "重新命名" }, { action: "copy-feature", icon: "⧉", label: "複製圖徵" }, { action: "duplicate-feature", icon: "＋", label: "複製一份" }, { action: "delete-feature", icon: "⌫", label: "刪除圖徵", danger: true }]); return; }
...
$("#contextMenu").addEventListener("click", async (event) => {
  ...
  else if(action==="zoom-reference"){ fitReferenceLayer(map,contextReferenceId); const input=document.querySelector<HTMLInputElement>(`[data-reference-layer="${contextReferenceId}"]`); if(input)input.checked=true; const group=referenceLayerDefinitions.find((item)=>item.id===contextReferenceId)?.group; if(group)syncReferenceGroup(group); }
  else if(action==="toggle-reference"){ const visible=!isReferenceLayerVisible(contextReferenceId); setReferenceLayerVisible(contextReferenceId,visible); const input=document.querySelector<HTMLInputElement>(`[data-reference-layer="${contextReferenceId}"]`); if(input)input.checked=visible; const group=referenceLayerDefinitions.find((item)=>item.id===contextReferenceId)?.group; if(group)syncReferenceGroup(group); }
  else if(action==="show-reference-group")setReferenceGroupVisible(contextReferenceGroup,true);
  else if(action==="hide-reference-group")setReferenceGroupVisible(contextReferenceGroup,false);
  else if(action==="reference-info"){ const definition=referenceLayerDefinitions.find((item)=>item.id===contextReferenceId); if(definition)alert(`${definition.group}／${definition.name}\n預設參考圖層 · WGS84 (EPSG:4326)\n僅供初步位置套疊與生態檢核參考。`); }
  else if (action === "zoom-selection" && contextFeature) fitFeatures(map, [contextFeature]);
  else if (action === "rename-feature" && contextFeature) { const name=prompt("圖徵名稱：",String(contextFeature.get("name")||"")); if(name?.trim()){ contextFeature.set("name",name.trim()); recordState("重新命名圖徵"); renderFeatureList(); populateProperties(contextFeature); } }
...
document.addEventListener("pointerdown", (event) => { if (!(event.target as HTMLElement).closest("#contextMenu")) closeContextMenu(); if(!(event.target as HTMLElement).closest(".desktop-menu-group")) document.querySelectorAll(".desktop-menu-group.open").forEach((item)=>item.classList.remove("open")); });
