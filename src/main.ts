...
import {
  ...
  setReferenceLayerVisible, toggleFeatureSelection, updateSelected, type EditorMode, type FeaturePreset, type LayerDefinition, type TbnRecord,
} from "./map";
    ...
    <aside class="workbench"><button id="workbenchResize" class="workbench-resize" title="拖曳調整左側寬度" aria-label="調整左側工作區寬度"></button>
      ...
      <section id="editorView" class="view-panel">
        ...
        <div class="editor-card reference-card">
          ...
          <div class="reference-layer-list">${[...new Set(referenceLayerDefinitions.map((item)=>item.group))].map((group)=>`<details><summary>${group}</summary>${referenceLayerDefinitions.filter((item)=>item.group===group).map((item)=>`<label><input type="checkbox" data-reference-layer="${item.id}"><i style="--reference-color:${item.color}"></i>${item.name}</label>`).join("")}</details>`).join("")}</div>
        </div>
        <div class="editor-card">
          <h3>資料交換</h3>
...
function renderLayoutLegend(): void { const lines=(<HTMLTextAreaElement>$("#layoutLegendText")).value.split(/\r?\n/).map((line)=>line.trim()).filter(Boolean); $("#layoutLegendPreview").innerHTML=`<strong>圖例</strong>${lines.map((line,index)=>`<span><i style="--legend-color:${["#f08a19","#153cff","#e32f35","#73a942"][index%4]}"></i>${escapeHtml(line)}</span>`).join("")}`; }
...
document.querySelectorAll<HTMLInputElement>("[data-reference-layer]").forEach((input)=>input.addEventListener("change",()=>setReferenceLayerVisible(input.dataset.referenceLayer!,input.checked)));
