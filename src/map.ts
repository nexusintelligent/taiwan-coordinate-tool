...
export function fitReferenceLayer(map:Map,id:string): void { const layer=referenceLayers.get(id); if(!layer)return; const source=layer.getSource(); const fit=()=>{ if(source?.getFeatures().length){ const extent=source.getExtent(); if (extent) map.getView().fit(extent,{padding:[70,70,70,70],maxZoom:16,duration:350}); } }; if(source?.getFeatures().length)fit(); else source?.once("featuresloadend",fit); layer.setVisible(true); }
